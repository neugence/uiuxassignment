import { CircleDashed, CopyPlus } from "lucide-react";
...
const InProgress = ({ openModal, setActiveCard, onDrop }) => {
  const {user} = useAuth()
  ...
  const pageCount = Math.ceil(inProgress?.length / itemsByPage);

  const currentItems = inProgress?.slice(
  ...
  return (
    <div>
      ...
      <div
        ...
        className="bg-yellow-500 text-black p-4 mb- cursor-pointer hover:scale-105 rounded-lg duration-300 transition-all ease-in-out"
      >
